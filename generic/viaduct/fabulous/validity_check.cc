...
bool CLBState::check_validity(Context *ctx, const LogicConfig &cfg, const CellTagger &cell_data, bool explain_invalid)
{
    ...
    auto check_ctrlsig = [&](unsigned idx, ControlSig actual, const ControlSetConfig &ctrl,
        ...
        if (ctrl.can_mask != -1) {
            ...
            if (actual.net == id___disconnected || (actual.net == id__CONST0 && ctrl.can_mask == 0) ||
                (actual.net == id__CONST1 && ctrl.can_mask == 1)) {
                return true;
            }
        }
...
bool BlockTracker::check_validity(Context *ctx, BelId bel, const FabricConfig &cfg, const CellTagger &cell_data,
                                  bool explain_invalid)
{
    ...
    const auto &entry = row.at(loc.x);
    if (flags.block == BelFlags::BLOCK_CLB) {
        return entry.clb->check_validity(ctx, cfg.clb, cell_data, explain_invalid);
    } else {
        return true;
    }
}
